...
# Webcam settings in sidebar for better space management
with st.sidebar:
    st.header("Webcam Detection Settings")
    conf_threshold_webcam = st.slider("Confidence Threshold", 0.0, 1.0, 0.5, 0.01, key="conf_threshold_webcam_slider")
    box_color_webcam_hex = st.color_picker("Bounding Box Color", "#00FF00", key="webcam_color_picker")
    box_color_rgb = tuple(int(box_color_webcam_hex.lstrip("#")[i:i+2], 16) for i in (0, 2, 4))
    box_color_bgr = (box_color_rgb[2], box_color_rgb[1], box_color_rgb[0])
    thickness_webcam = st.slider("Bounding Box Thickness", 1, 10, 4, key="thickness_webcam_slider")
...
class VideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        # Update params from Streamlit's session state
        self.confidence_threshold = conf_threshold_webcam
        self.box_color = box_color_bgr
        self.thickness = thickness_webcam
        self.show_confidence = show_confidence
